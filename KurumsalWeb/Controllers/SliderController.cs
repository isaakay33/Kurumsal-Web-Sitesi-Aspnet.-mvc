    ...
    public class SliderController : Controller
    {
        ...
        public ActionResult Edit([Bind(Include = "SliderId,Baslik,Aciklama,ResimURL")] Slider slider,HttpPostedFileBase ResimURL,int id)
        {
            if (ModelState.IsValid)
            {
                ...
                if (ResimURL != null)
                {
                    if (System.IO.File.Exists(Server.MapPath(s.ResimURL)))/// eski resim varmı 
                    {
                        System.IO.File.Delete(Server.MapPath(s.ResimURL)); // eski resmi sil
                    }
        ...
        {
            Slider slider = db.Slider.Find(id);
            if (System.IO.File.Exists(Server.MapPath(slider.ResimURL)))/// eski resim varmı 
            {
                System.IO.File.Delete(Server.MapPath(slider.ResimURL)); // eski resmi sil
            }
